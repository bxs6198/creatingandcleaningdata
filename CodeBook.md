...
### Overview ###
This CodeBook explains the `tidydataset.csv` data set generated by the `run_analysis.R` script.  It contains the  names of variables, a quick explanation of what they are, and the type of the variable (char, integer, etc.).

#### Data Structure ####
`tidydataset.csv` is a comma separated values text file with the following structure.
