...
3. A tidy data set textfile `tidydataset.csv` containing comma separated values with the average of each variable for each activity and each subject.
...
5. A link to a Github repository with the `README.md` file, the `run_analysis.R` script for performing the analysis, and the resulting tidy data set file `tidydataset.csv`.
...
### Output Data ###
The result of the data analysis is a tidy data set conforming to the conditions identified in the Week 3 Lecture titled "Reshaping data" by Jeffrey Leek, which in turn originated in the paper 
...
The tidy data set `tidydataset.csv` generated by `run_analysis.R` meets all those conditions.  This file is a comma separated values text file that can be imported into RStudio using `read.csv`. Alternately, the file can be imported into a spreadsheet program such as Microsoft Excel.
